...
# Type check and simple test for tictactoe_functions.get_str_index
result = tictactoe_functions.get_str_index(1, 1, 3)
assert isinstance(result, int), \
    'tictactoe_functions.get_str_index should return an int, but returned ' \
    '{0}.'.format(type(result))
assert result == 0, \
    'tictactoe_functions.get_str_index(1, 1, 3) should return 0, but ' \
    'returned {0}.'.format(result)
